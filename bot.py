...
async def analyze_bouquet(photo_bytes: bytes):
    ...
    prompt = (
        "📸 выступи в роли профессионального флориста и Проанализируй фото букета и дай коротко несколько ответов по фотографии:\n"
        "🌸 Какие в нем цветы и их количество (в одном пункте), если есть зелень, укажи какая именно, если узнаешь. цветы указывай тоже максимально конкретно вместе с цветом.  \n"
        "💰 Средняя стоимость букета в рублях, не диапазон, а конкретно среднюю цену, коротко.\n"
        "Используй эмодзи, делай текст лёгким, понятным, доброжелательным, ты милая девушка флорист."
    )
